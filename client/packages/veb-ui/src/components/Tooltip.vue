<template>
    <span class="tooltip-wrapper">
        <slot></slot>
        <div class="veb-tooltip">{{text}}</div>
    </span>
</template>
<script>
...
export default {
    ...
    mounted() {
        ...
        self.$nextTick(()=> {
            ...
            self.childElement = self.$el.querySelector(".veb-tooltip")
    ...
    methods: {
        reposition() {
            ...
            let elemRect = this.triggerElement.getBoundingClientRect()
            let offsetTop   = elemRect.top - bodyRect.top
            let offsetLeft = elemRect.left - bodyRect.left
            let offsetRight = bodyRect.right - elemRect.right
            let offsetBottom = bodyRect.bottom - elemRect.bottom
